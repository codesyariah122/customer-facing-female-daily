...
const ModalMapEdit = (props: any) => {
  ...
  // useeffect get default lat lang
  useEffect(() => {
    props.dataCurrent?.latitude !== undefined &&
      setDefaultLatLng({
        lat: props.dataCurrent?.latitude,
        lng: props.dataCurrent?.longitude,
      });
    // ---
  }, [props.dataCurrent?.latitude, props.dataCurrent?.longitude]);
  // --
  ...
  // fn use this location
  const useThisLocFn = () => {
    isChange &&
      (props.closeModal(), props.useThisLoc(dataAddressCurrent, defaultLatLng));
  };
  // ---
                    ...
                    <div className="flex-1 pr-3">
                      <div className="flex flex-col">
                        <strong className="text-lg font-semibold tracking-wider">
                          {dataLoc.label || props.dataCurrent?.subdistrict}
                        </strong>
                        <span className="text-black-light tracking-wider">
                          {dataLoc.address || props.dataCurrent?.address}
                        </span>
                      </div>
                    </div>
                    <div>
                      <div
                        className={`${
                          isChange
                            ? 'bg-pink-primary'
                            : 'bg-black-lighter pointer-events-none'
                        } bg-pink-primary flex w-[168px] cursor-pointer justify-center rounded py-4 text-sm font-semibold tracking-wider text-white`}
                        onClick={useThisLocFn}
                      >
