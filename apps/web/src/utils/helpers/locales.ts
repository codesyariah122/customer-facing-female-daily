 ...
 * @license Proprietary
 * @package utils/helpers/locales
 */

import { I18n, I18nLocales } from '@utils/i18n';
...
type CreateContentLocales<K extends string> = <T = I18nLocales>(
  locale: T
) => Record<K, string>;
 ...
 */
export function generateLocaleBasedContents<T extends string>(
  data: I18nContentToLocales<T>
): I18nLocalesToContent<T> {
  const classificationLocales: I18nLocalesToContent<T> =
    Object.entries<ObjectContents>(data).reduce(
      (accumulator, [key, contentLocales]) => {
 ...
 * @param data (content->locales based) data
 */
export function createContentLocales<T extends string>(
  data: I18nContentToLocales<T>
): CreateContentLocales<keyof typeof data> {
